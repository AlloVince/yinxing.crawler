...
domain = os.environ.get('DOMAIN') or 'sehuatanglu.com'
...
class BailuSpider(BaseSpider):
    ...
    rules = (
        Rule(LinkExtractor(allow=r'forum-(2|36|37|103|104|38|39)-(1|2|3|4)\.html', ), follow=True),
        Rule(LinkExtractor(allow=r'thread-\d+-1-\d+.html', ), follow=True, callback='handle_page'),
    )

    deep_rules = (
        Rule(LinkExtractor(allow=r'forum-(2|36|37|38|55|76|77)-[-\w]+\.html', ), follow=True),
        Rule(LinkExtractor(allow=r'thread-\d+-1-\d+.html', ), follow=True, callback='handle_page'),
    )
